...
1. Trigger on sensitive sources in added lines, such as `os.environ`, `git remote get-url`, home-directory config reads, `.netrc`, `*.pem`, `*.key`, `.ssh`, `.aws`, keyring access, or auth/session/webhook/secret-manager payloads and request/response headers carrying cookies, tokens, or authorization material.
...
- Per-PR head-to-head in the regular calibration corpus now uses the corrected structured join: 207 parsed PRs, tied-rank Spearman `0.089–0.463`, and Pearson `0.250–0.383`. Treat this as pattern-overlap evidence only, not independent calibration or promotion evidence. Whole-repo per-KLOC still only weakly agrees because the rule sets target different patterns.
...
| Verification mode | universal | `contract_errors(...)`, possibly `parser()`/`declare(...)` only after calibration | First pass: require `proof_plan` prose to name `red-green-refactor`, `green-refactor-green`, or `smoke-check` for non-minimal code work. Later pass, if stable, add `--verification-mode` and store it in `declare(...)`. |
| Production-shaped proof | universal | `final_errors(...)`, `run_quality_gate(...)` warning helper over changed test files | For bugfixes and behavior changes, warn when tests changed but no added test line appears to call a production entrypoint, CLI/hook command, local server, parser, or production-shaped fixture. Keep as warning because language-specific precision varies. |
| Wasteful test bulk | universal | `run_quality_gate(...)`, new `test_shape_warnings(ctx)` helper, `quality_checks(...)` | Warn only on high-confidence combinations: large added test block, high mock/setup token ratio, weak assertion count, and no visible production entrypoint call. Avoid line-count-only failures. |
| Behavioral contracts and out-of-scope boundaries | universal | `contract_errors(...)` over existing `authoritative_contract`, `affected_surface`, `invariants`, and `risk_check` | Add lightweight text checks for procedural placeholders only after reviewing real contracts. Do not reject file paths categorically; reject contracts that only say "edit file X/line Y" without desired behavior or acceptance criteria. |
| Deterministic helpers and debug probes | universal | Existing temp-artifact and quality-escape checks plus a small added-line warning helper | Warn on new scripts/harnesses/debug markers unless referenced by `verify`, named in `risk_check`, or clearly temporary and removed before final. Continue relying on final quality gate to catch lingering temp artifacts. |
| Delta reporting | universal | `run_quality_gate(...)` JSON result | Add additive fields such as `securityAssumptionFindings`, `slopShapeFindings`, and `verificationShapeFindings`, each grouped by `added`, `resolved`, `worsened`, and `improved` when baseline comparison is available. Existing consumers of `ok`, `errors`, `warnings`, and `checks` remain compatible. |
