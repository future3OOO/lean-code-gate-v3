...
Advisory findings are bucketed as added, resolved, improved, or worsened for touched files. Warnings are emitted for moderate bloat and weaker reuse signals. Projects can promote legacy warnings to failures in `.agent/lean/policy.json`; advisory findings stay out of that promotion path. Optional reward telemetry is disabled by default and records only aggregate Stop-time quality, delta, and advisory counts when explicitly enabled; no score, challenge, leaderboard, badge, streak, or per-agent identity is emitted.
