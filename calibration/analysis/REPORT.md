...
Five calibration deltas (R-1 through R-6) cover all four classes. They are conservative — every change is policy-overridable per repo. Landing them across PR-5..PR-8 would reduce the gate's error-tier FP rate from ~26% on merged PRs to an estimated ~5%, while preserving all observed TPs and not introducing new false negatives in the data we measured.
